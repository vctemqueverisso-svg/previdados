    ...
    <div className="space-y-8">
      <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <div className="card p-7">
          ...
          <h1 className="mt-3 max-w-4xl text-[2.45rem] font-semibold leading-[0.96] tracking-[-0.04em] text-ink md:text-[3.4rem]">
            Controle da carteira, dos atendimentos e do andamento processual em um único painel.
          </h1>
          <p className="mt-4 max-w-3xl text-[15px] leading-7 text-slate-600">
            Aqui você acompanha os indicadores do escritório e enxerga com mais clareza os resultados, os tempos médios e o perfil da carteira.
          </p>
        ...
        <div className="card p-7">
          ...
          <div className="mt-5 grid gap-4">
            <div className="rounded-[24px] border border-[rgba(24,38,63,0.08)] bg-[linear-gradient(180deg,rgba(21,34,56,0.98),rgba(17,28,46,0.98))] px-5 py-5 text-white">
              ...
              <p className="mt-3 text-sm leading-6 text-slate-200">Clientes com cadastro pronto para atendimento, estratégia e vinculação de casos.</p>
            </div>

            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-[24px] border border-[rgba(24,38,63,0.08)] bg-[rgba(248,251,255,0.92)] px-5 py-5">
                <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500">Perícia</p>
                <p className="mt-3 text-[2rem] font-semibold leading-none text-ink">{data.totals.averageDaysToExpertise} dias</p>
                <p className="mt-3 text-sm text-slate-600">Tempo médio até a perícia.</p>
              </div>
              <div className="rounded-[24px] border border-[rgba(24,38,63,0.08)] bg-[rgba(248,251,255,0.92)] px-5 py-5">
                <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500">Decisão</p>
                <p className="mt-3 text-[2rem] font-semibold leading-none text-ink">{data.totals.averageDaysToDecision} dias</p>
                <p className="mt-3 text-sm text-slate-600">Tempo médio até a decisão.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="grid gap-6 xl:grid-cols-[0.92fr_1.08fr]">
        <div className="card p-6">
          <p className="eyebrow">Gestão da carteira</p>
          <h2 className="mt-2 text-[2rem] font-semibold leading-[1.02] tracking-[-0.03em] text-ink">Quadro de desempenho</h2>
          <p className="mt-3 text-sm leading-6 text-slate-600">
            Visão compacta para saber rapidamente como a carteira está performando e onde você pode concentrar esforço.
          </p>

          <div className="mt-6 space-y-4">
            <div className="rounded-[24px] border border-[rgba(24,38,63,0.08)] bg-[rgba(248,251,255,0.92)] px-5 py-5">
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Procedência</p>
              <p className="mt-3 text-[2.3rem] font-semibold leading-none text-ink">{data.totals.successfulCases}</p>
              <p className="mt-3 text-sm text-slate-600">Casos com desfecho favorável no resultado final registrado.</p>
            </div>
            <div className="rounded-[24px] border border-[rgba(24,38,63,0.08)] bg-[rgba(248,251,255,0.92)] px-5 py-5">
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Improcedência</p>
              <p className="mt-3 text-[2.3rem] font-semibold leading-none text-ink">{data.totals.unsuccessfulCases}</p>
              <p className="mt-3 text-sm text-slate-600">Casos com desfecho desfavorável, relevantes para leitura de risco e tese.</p>
            </div>
          </div>
        </div>
        ...
        <div className="card p-6">
          ...
          <h2 className="mt-2 text-[2rem] font-semibold leading-[1.02] tracking-[-0.03em] text-ink">Casos por benefício</h2>
          <p className="mt-2 text-sm leading-6 text-slate-500">Distribuição da carteira para leitura rápida do perfil de atendimento do escritório.</p>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <div className="card p-6">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">Comparação entre a frente administrativa e a judicial para acompanhar o perfil do contencioso.</p>
          <SimpleBarChart data={data.byChannel} color="#6f86ac" />
        </div>

        <div className="card p-6">
          <p className="eyebrow">Resultado</p>
          <h2 className="mt-2 text-[2rem] font-semibold leading-[1.02] tracking-[-0.03em] text-ink">Êxito por doença</h2>
          <p className="mt-2 text-sm leading-6 text-slate-500">Comparativo entre desfechos favoráveis e desfavoráveis por quadro patológico registrado.</p>
